<?php
	...
	Class Feed {
		...
		function add($feedURL, $visibility = 'y', $filter = '', $filterType = 'tag', $group = '0', $owner = 0) {
			...
			if (!$db->execute('INSERT INTO '.$database['prefix'].'Feeds (owner, `group`, xmlURL, xmlType, blogURL, title, description, language, lastUpdate, created, visibility, filter, filterType, logo) VALUES ("'.$owner.'", "'.$group.'", "'.$feedInfo['xmlURL'].'", "'.$feedInfo['xmlType'].'", "'.$feedInfo['blogURL'].'", "'.$feedInfo['title'].'", "'.$feedInfo['description'].'", "'.$feedInfo['language'].'", 0, UNIX_TIMESTAMP(), "'.$db->escape($visibility).'","'.$db->escape($filter).'", "'.$filterType.'", "'.$feedInfo['logo'].'")')) {
				return false;
			}	
