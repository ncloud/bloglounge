<?php
	...
	if(isset($_POST['feedURL']) && !empty($_POST['feedURL'])) { 
		...
		if($userInformation['is_accepted'] == 'y') {
			...
			if($addUser=='-1' && empty($username)) $addUser = '0'; 
...
?>		
	...
	<div class="wrap">
		<div class="warning_messages_wrap">
			<br />
				<?php echo _f('<strong>[%1]</strong>는 이미 등록된 회원의 아이디이거나 잘못된 형식의 아이디입니다.', $add_user_error);?>
			<br /><br />
...
?>
	<div class="wrap add_detail_wrap">
		<div class="messagebox">
			<a href="./"><?php echo _t('다른 피드를 추가 하시려면 이곳을 클릭하십시오.');?></a>
		</div>
...
?>
					...
					<dl>
						<dt><?php echo _t('대리');?></dt>
						<dd>
							<select id="addUser" name="addUser" onchange="if($('#addUser').val() == '-1') $('#username').show().focus(); else $('#username').hide();"  align="absmiddle">
								<option value="0"><?php echo _t('사용안함');?></option>		
								<option value="-1"><?php echo _t('새로운 회원 추가');?></option>
...
?>
							...
							<input type="text" id="username" name="username" class="input adduser faderInput" value="" align="absmiddle" />

							<div class="help">
								<?php echo _t('위 옵션을 선택하시면 선택된 회원 또는 새롭게 추가한 회원의 권한으로 블로그를 등록하게 됩니다.');?>
							</div>
						</dd>
					</dl>
